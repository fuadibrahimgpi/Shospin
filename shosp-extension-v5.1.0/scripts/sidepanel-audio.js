...
async function processAudioInBackground(audioBlob, capturedText) {
  ...
  try {
    ...
    if (data && data.dados) {
      ...
      // v5.1: passar a resposta + estado de hooks pelo DecisionEngine para
      // produzir um log auditável dos guards aplicados. Não substitui o
      // pipeline existente abaixo (mantido para preservar comportamento
      // clínico), mas torna visível em log único quais regras dispararam.
      try {
      ...
      // Sincronizar flags de exames entre gancho (voz) e agente (IA).
      //
      // REGRA: gancho disparado durante gravação → injeção direta já ocorreu →
      //        suprimir flag no injectAll para EVITAR DUPLA INJEÇÃO.
      //        Gancho NÃO disparou → agente detectou → injectAll injeta uma vez.
      //
      // Prioridade (exclusão mútua): blefarite > exames normais.
      if (blefariteInjetada) {
        // Gancho injetou biomicroscopia/fundoscopia durante gravação — não reinjetar.
        data.injectBlefarite   = false;
        data.injectExamesNormais = false;
      } else if (examesNormaisInjetados) {
        // Gancho injetou exames normais durante gravação — não reinjetar.
        data.injectExamesNormais = false;
        data.injectBlefarite   = false;
      } else if (data.injectBlefarite) {
        // Agente detectou blefarite; gancho não disparou → injectAll vai injetar.
        data.injectExamesNormais = false;
      } else if (data.injectExamesNormais) {
        // Agente detectou exames normais; gancho não disparou → injectAll vai injetar.
        data.injectBlefarite = false;
      }
      ...
      // Fallback: verificar dilatação no texto processado + buffer recente
      ...
      // Omitir conduta e tratamento se for pausa para dilatação/espera
      if (dilatacaoDetectada && data.dados) {
        data.dados.conduta = null;
        data.dados.tratamento = null;
        if (data.dados.medicamentos_prescritos !== undefined) data.dados.medicamentos_prescritos = null;
        console.log('[Audio] Conduta omitida devido à flag de dilatação/espera.');
      }

      // Se dilatação detectada E não há nenhum dado clínico capturado,
      // é um comando puro de pausa — não exibir nem injetar nada.
      if (dilatacaoDetectada && !_temDadosClinicos(data.dados)) {
        ...
        console.log('[Audio] Dilatação pura — sem dados para exibir ou injetar.');
        return;
      }
